...
const FactoryInventory = () => {
  ...
  const fetchFactories = async () => {
    try {
      const response = await fetch(API_ROUTES.FACTORY_INVENTORY_FACTORIES, {
        headers: {
  ...
  const fetchSummary = async (factoryId) => {
    try {
      const response = await fetch(API_ROUTES.FACTORY_INVENTORY_SUMMARY(factoryId), {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleFactoryChange = (event) => {
    setSelectedFactory(Number(event.target.value));
    setCurrentPage(1);
  };
  ...
  const saveEditModal = async () => {
    if (!editModal.data || !selectedFactory) return;
    try {
      setSavingEdit(true);
      setError(null);
      const response = await fetch(API_ROUTES.FACTORY_INVENTORY_ITEM_UPDATE(selectedFactory), {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const exportToCSV = () => {
    ...
    const csvData = filteredInventory.map(item => [
      ...
      getStockStatus(item.stock, Number(item.alert_quantity) || 10).text
    ]);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-2">Manage factory materials and products inventory</p>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            <div className="w-full md:w-1/3">
              ...
              <select
                ...
                className="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/30 disabled:opacity-50"
              >
            ...
            {summary && (
              <div className="flex-1 flex flex-wrap gap-3">
                <div className="flex items-center gap-2 px-4 py-2 bg-blue-100/50 backdrop-blur-sm border border-white/40 rounded-xl">
                  <Package size={18} className="text-blue-600" />
                  <span className="text-sm font-medium text-gray-700">
                    Materials: {summary.materials.count}
                  </span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-purple-100/50 backdrop-blur-sm border border-white/40 rounded-xl">
