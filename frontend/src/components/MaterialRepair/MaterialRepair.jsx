...
const MaterialRepair = () => {
  ...
  const fetchRepairs = async () => {
    try {
      setLoading(true);
      const response = await axios.get(API_ROUTES.MATERIAL_REPAIRS, getAuthHeaders());
      setRepairs(response.data.repairs || response.data || []);
    } catch (error) {
      console.error('Error fetching material repairs:', error);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      // Prepare returned materials data
      const returnedMaterials = selectedRepair.materialRepairItem.map(item => ({
      ...
      const response = await axios.patch(
        `${API_ROUTES.MATERIAL_REPAIRS}/${selectedRepair.id}/status`,
        {
          status: 'completed',
          returnedMaterials
        },
      ...
      alert('Material repair status updated successfully!');
      setShowStatusModal(false);
      fetchRepairs(); // Refresh the list
    } catch (error) {
      console.error('Error updating material repair status:', error);
      alert(error.response?.data?.error || 'Failed to update material repair status');
    } finally {
  ...
  const openStatusModal = (repair) => {
    ...
    repair.materialRepairItem.forEach(item => {
  ...
  const analytics = {
    ...
    totalMaterialsSent: repairs.reduce((sum, r) => sum + (r.materialRepairItem?.length || 0), 0),
    totalUnits: repairs.reduce((sum, r) => 
      sum + (r.materialRepairItem?.reduce((itemSum, item) => itemSum + (item.quantity || 0), 0) || 0), 0),
    ...
    completionRate: repairs.length > 0 
      ...
      : 0,
    averageRepairCost: repairs.length > 0 
      ? (repairs.reduce((sum, r) => sum + (r.shippingCost || 0), 0) / repairs.length).toFixed(2)
      : 0
  };
  ...
  const exportToCSV = () => {
    const headers = ['ID', 'Source Type', 'Source ID', 'Destination', 'Status', 'Materials Count', 'Total Units', 'Shipping Cost', 'Created Date', 'Note'];
    const csvData = repairs.map(repair => [
      ...
      repair.materialRepairItem?.length || 0,
      repair.materialRepairItem?.reduce((sum, item) => sum + (item.quantity || 0), 0) || 0,
    ...
    a.href = url;
    a.download = `material-repairs-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };

  const calculateMaterialStats = (repair) => {
    const items = repair.materialRepairItem || [];
    const totalQuantity = items.reduce((sum, item) => sum + (item.quantity || 0), 0);
    const totalSuccess = items.reduce((sum, item) => sum + (item.success || 0), 0);
    const totalFail = items.reduce((sum, item) => sum + (item.fail || 0), 0);
    const successRate = totalQuantity > 0 ? (totalSuccess / totalQuantity * 100).toFixed(1) : 0;
    ...
    return {
      ...
      totalFail,
      successRate,
      pending: totalQuantity - totalSuccess - totalFail
    };
  };
        ...
        <div className="bg-white rounded-2xl shadow-lg mb-6 p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">Material Repair Management</h1>
                <p className="text-gray-600">Track and manage material repair requests</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              <button
                onClick={() => navigate('/addrepairmaterial')}
                className="px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all shadow-lg flex items-center gap-2"
              >
                <Truck size={20} />
                New Repair Request
              </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-6">
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-orange-600 font-medium">Total Repairs</p>
                  <p className="text-2xl font-bold text-orange-700">{analytics.totalRepairs}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-purple-600 font-medium">Materials Sent</p>
                  <p className="text-2xl font-bold text-purple-700">{analytics.totalMaterialsSent}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-amber-600 font-medium">Total Units</p>
                  <p className="text-2xl font-bold text-amber-700">{analytics.totalUnits.toFixed(2)}</p>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search repairs, materials, or brands..."
                className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/30 focus:border-orange-300"
              />
            </div>
            ...
            <div>
              <select
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/30"
              >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-4 py-3 bg-orange-500 text-white font-medium rounded-xl hover:bg-orange-600 transition-colors flex items-center justify-center gap-2"
              >
                <RefreshCw size={18} />
                Refresh
              </button>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-700 mb-2">No Material Repair Requests Found</h3>
              <p className="text-gray-600 mb-6">Try adjusting your filters or create a new repair request</p>
              <button
                onClick={() => navigate('/addrepairmaterial')}
                className="px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all"
              >
              ...
              <table className="w-full">
                <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                  <tr>
                    ...
                    <th className="p-4 text-left font-semibold text-gray-700">Materials</th>
                    <th className="p-4 text-left font-semibold text-gray-700">Status</th>
                    <th className="p-4 text-left font-semibold text-gray-700">Stats</th>
                ...
                <tbody>
                  {filteredRepairs.map((repair) => {
                    const stats = calculateMaterialStats(repair);
                        ...
                        <td className="p-4">
                          <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium">{repair.materialRepairItem?.length || 0} items</span>
                            </div>
                            <div className="text-xs text-gray-500">
                              {stats.totalQuantity.toFixed(2)} units
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                          ...
                          <div className="text-xs">
                            <div className="flex items-center gap-2 mb-1">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-green-600">{stats.totalSuccess.toFixed(2)}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <div className="w-2 h-2 rounded-full bg-red-500"></div>
                                <span className="text-red-600">{stats.totalFail.toFixed(2)}</span>
                              </div>
                            </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">Material Repair Request Details</h2>
            ...
            <div className="overflow-y-auto max-h-[calc(90vh-200px)]">
              <div className="p-6">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-4">Materials Sent for Repair</h3>
                  <div className="overflow-hidden rounded-xl border border-gray-200">
                    <table className="w-full">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="p-3 text-left text-sm font-medium text-gray-700">Material</th>
                          <th className="p-3 text-left text-sm font-medium text-gray-700">Brand</th>
                      ...
                      <tbody>
                        {selectedRepair.materialRepairItem?.map((item) => {
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    {item.material?.barcode && (
                                      <div className="text-xs text-gray-500">Code: {item.material.barcode}</div>
                                    )}
                                  </div>
                                </div>
                              </td>
                              <td className="p-3">
                                <span className="text-sm text-gray-700">{item.material?.brand || 'N/A'}</span>
                              </td>
                              <td className="p-3 font-medium">{item.quantity.toFixed(2)}</td>
                              <td className="p-3">
                                <span className="text-sm text-gray-600">{item.material?.unit || 'units'}</span>
                              </td>
                              <td className="p-3">
                                <div className="flex items-center gap-2">
                      ...
                      <tfoot className="bg-gray-50">
                        <tr>
                          <td colSpan="2" className="p-3 text-right font-medium text-gray-700">
                            Totals:
                          </td>
                          <td className="p-3 font-bold text-gray-800">
                            {selectedRepair.materialRepairItem?.reduce((sum, item) => sum + (item.quantity || 0), 0).toFixed(2)}
                          </td>
                          <td className="p-3"></td>
                          <td className="p-3 font-bold text-green-600">
                            {selectedRepair.materialRepairItem?.reduce((sum, item) => sum + (item.success || 0), 0).toFixed(2)}
                          </td>
                          <td className="p-3 font-bold text-red-600">
                            {selectedRepair.materialRepairItem?.reduce((sum, item) => sum + (item.fail || 0), 0).toFixed(2)}
                          </td>
                          <td className="p-3 font-bold text-yellow-600">
                            {selectedRepair.materialRepairItem?.reduce((sum, item) => sum + (item.quantity - (item.success || 0) - (item.fail || 0)), 0).toFixed(2)}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">Update Material Repair Status</h2>
            ...
            <div className="overflow-y-auto max-h-[calc(90vh-200px)] p-6">
              <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-xl">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-orange-800">Enter success and fail quantities for each material</p>
                    <p className="text-xs text-orange-700 mt-1">
                      Successfully repaired materials will be added back to stock. Failed materials remain as scrap.
                      You can use decimal values for material quantities.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div key={item.id} className="p-4 border border-gray-200 rounded-xl">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-gray-800">{item.material?.name || 'Unknown Material'}</div>
                            <div className="text-sm text-gray-600">
                              Brand: {item.material?.brand || 'N/A'} • Max: {maxQuantity.toFixed(2)} {item.material?.unit || 'units'}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100">
                        <div className="flex justify-between text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span>Success: {currentSuccess.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full bg-red-500"></div>
                            <span>Fail: {currentFail.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                            <span>Pending: {remaining.toFixed(2)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="p-6 border-t border-gray-200 bg-gray-50">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-green-600">
                        {Object.values(successQuantities).reduce((sum, val) => sum + (parseFloat(val) || 0), 0).toFixed(2)} Success
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <XCircle size={16} className="text-red-500" />
                      <span className="font-bold text-red-600">
                        {Object.values(failQuantities).reduce((sum, val) => sum + (parseFloat(val) || 0), 0).toFixed(2)} Fail
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={updatingStatus}
                  className="px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {updatingStatus ? (
                    <>
