...
const AllShop = () => {
  ...
  const canEditShop = hasPermission('shop_edit');
  const canDeleteShop = hasPermission('shop_delete');
    ...
    const fetchShops = async () => {
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          alert('Permission denied. You do not have access to shops.');
        }
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this shop?')) {
      try {
        await axios.delete(`${API_ROUTES.SHOPS}/${id}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        
        // Refetch shops after deletion
        const response = await axios.get(API_ROUTES.SHOPS, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
        ...
        if (error.response?.status === 401) {
          ...
          alert('Permission denied. You cannot delete shops.');
        } else {
  ...
  const openDetailsModal = (shop) => {
    setModal({ isOpen: true, type: 'details', data: shop });
  };
  ...
  const getContactStatus = (shop) => {
    const hasPhone = shop.mobile && shop.mobile.trim() !== '';
    const hasKeeper = shop.shop_keeper && shop.shop_keeper.trim() !== '';
  ...
  const shopsWithKeeper = shops.filter(s => s.shop_keeper && s.shop_keeper.trim() !== '').length;
  const shopsWithPhone = shops.filter(s => s.mobile && s.mobile.trim() !== '').length;
  ...
  return (
    <div className="min-h-screen rounded-t-2xl w-full bg-gradient-to-br from-fuchsia-50 via-pink-50 to-rose-50 p-4 md:p-6">
      {/* Background decorative elements */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-fuchsia-300/20 rounded-full blur-3xl"></div>
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-pink-300/20 rounded-full blur-3xl"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-rose-300/10 rounded-full blur-3xl"></div>
      </div>

      <div className="relative w-full mx-auto">
        {/* Header Card */}
        <div className="backdrop-blur-xl bg-white/40 border border-white/60 rounded-2xl shadow-2xl shadow-fuchsia-100/50 mb-6 p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="p-4 bg-gradient-to-br from-fuchsia-500 to-pink-500 rounded-2xl shadow-lg">
                <ShoppingBag className="text-white" size={36} />
              </div>
              <div>
                <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-fuchsia-600 to-pink-600 bg-clip-text text-transparent">
                  All Shops
                </h1>
                <p className="text-gray-600 mt-2">Manage your retail shop locations and contacts</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              { canCreateShop && (
                <Link 
                  to="/shop/add" 
                  className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-600 hover:to-pink-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  <Plus size={20} />
                  New Shop
                </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          <div className="backdrop-blur-lg bg-gradient-to-br from-fuchsia-50/60 to-pink-50/60 border border-white/40 rounded-2xl shadow-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Total Shops</p>
                <p className="text-2xl font-bold text-fuchsia-600">{totalShops}</p>
              </div>
              <div className="p-3 bg-fuchsia-100 rounded-xl">
                <Store size={24} className="text-fuchsia-600" />
              </div>
            </div>
          </div>
          ...
          <div className="backdrop-blur-lg bg-gradient-to-br from-emerald-50/60 to-green-50/60 border border-white/40 rounded-2xl shadow-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">With Shop Keeper</p>
                <p className="text-2xl font-bold text-emerald-600">{shopsWithKeeper}</p>
              </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex flex-col items-center gap-4">
                <div className="w-12 h-12 border-4 border-fuchsia-500/30 border-t-fuchsia-600 rounded-full animate-spin"></div>
              ...
              <div className="overflow-x-auto rounded-xl border border-white/60">
                <table className="w-full text-sm">
                  <thead className="bg-gray-100/80">
                    <tr>
                      <th className="p-4 text-left font-medium text-gray-700">Shop Info</th>
                  ...
                  <tbody>
                    {currentShops.map((shop, index) => {
                      const contactStatus = getContactStatus(shop);
                      
                      return (
                        <tr key={shop.id} className={`border-t border-white/50 hover:bg-white/30 transition-colors duration-200 ${
                          index % 2 === 0 ? 'bg-white/10' : ''
                        }`}>
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 bg-gradient-to-br from-fuchsia-100 to-pink-100 flex items-center justify-center">
                                <ShoppingBag size={20} className="text-fuchsia-600" />
                              </div>
                              <div>
                                <p className="font-semibold text-gray-800">{shop.name}</p>
                                <div className="flex items-center gap-2 mt-1">
                                  <User size={12} className="text-gray-400" />
                                  <p className="text-xs text-gray-500">{shop.shop_keeper || 'No keeper'}</p>
                                </div>
                              </div>
                            </div>
                          </td>
                            ...
                            <div className="flex flex-col items-start gap-2">
                              ...
                              <div className="text-sm text-gray-700">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>{shop.mobile || 'No phone'}</span>
                                </div>
                          ...
                          <td className="p-4">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                <p className="font-medium text-gray-800">{shop.address || 'No address'}</p>
                                {shop.address && (
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className="p-2 bg-fuchsia-50 text-fuchsia-600 rounded-lg hover:bg-fuchsia-100 transition-colors duration-300"
                                title="View Details"
                              >
                                <Eye size={16} />
                              </button>
                ...
                {shops.length === 0 && !loading && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-6">Get started by adding your first retail shop</p>
                    <Link 
                      to="/shops/add" 
                      className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300"
                    >
                      <Plus size={20} />
                      Add First Shop
                    </Link>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          className="text-sm border border-gray-300 rounded-lg px-3 py-1.5 bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-500/30"
                        >
                      ...
                      <div className="text-sm text-gray-700">
                        ...
                        of <span className="font-semibold">{totalShops}</span> shops
                      </div>
                    </div>
      ...
      {modal.isOpen && modal.type === 'details' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative backdrop-blur-xl bg-white/95 border border-white/60 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div className="sticky top-0 z-10 p-6 border-b border-white/50 bg-gradient-to-r from-fuchsia-500/10 to-pink-500/10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                    ...
                    <div>
                      ...
                      {modal.data.address && (
                        <p className="text-sm text-gray-500 mt-2">
                          Full shop location and address information
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="backdrop-blur-sm bg-white/50 border border-white/40 rounded-xl p-5 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-gradient-to-r from-fuchsia-50/50 to-fuchsia-100/30 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      <User size={16} className="text-fuchsia-600" />
                      <p className="text-sm font-medium text-gray-700">Shop Keeper</p>
                    </div>
                    <p className="text-lg font-bold text-fuchsia-600">
                      {modal.data.shop_keeper || 'Not assigned'}
                    </p>
                  </div>
                  <div className="p-4 bg-gradient-to-r from-pink-50/50 to-pink-100/30 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="p-4 bg-gradient-to-r from-purple-50/50 to-purple-100/30 rounded-lg">
                    ...
                    <p className="text-lg font-bold text-purple-600">
                      {modal.data.shopProducts?.length || 0}
                    </p>
                  </div>
                </div>
              </div>
