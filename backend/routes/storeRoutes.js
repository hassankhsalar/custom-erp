...
router.get('/:id/inventory/list', async (req, res) => {
  ...
  try {
    ...
    const store = await prisma.store.findFirst({
      where: { id: storeId, deleted_at: false },
      include: {
        storeProducts: { where: { deleted_at: false, product: { deleted_at: false } }, include: { product: true } },
        storeMaterials: { where: { deleted_at: false, material: { deleted_at: false } }, include: { material: true } },
      },
    });
    if (!store) return res.status(404).json({ error: 'Store not found' });

    const products = (store.storeProducts || []).map((sp) => ({
      ...
      unit_cost: sp.unit_cost, sale_price: sp.sale_price ?? sp.product?.sale_price, alert_quantity: sp.alert_quantity ?? sp.product?.alert_quantity, description: sp.product?.description, category: sp.product?.category, barcode: sp.product?.barcode, brand: sp.product?.brand
    }));
    const materials = (store.storeMaterials || []).map((sm) => ({
      ...
      unit_cost: sm.material?.unit_cost, sale_price: sm.sale_price ?? sm.material?.sale_price, alert_quantity: sm.alert_quantity ?? sm.material?.alert_quantity, description: sm.material?.description, category: sm.material?.category, barcode: sm.material?.barcode, brand: sm.material?.brand
    }));
    ...
    if (error.status === 403) return res.json({ items: [], pagination: { page: 1, limit: 20, totalCount: 0, totalPages: 1 } });
    res.status(500).json({ error: 'Failed to fetch store inventory list' });
  }
});
...
router.get('/:id/inventory/summary', async (req, res) => {
  ...
  try {
    ...
    const store = await prisma.store.findFirst({
      where: { id: storeId, deleted_at: false },
      include: {
        storeProducts: { where: { deleted_at: false, product: { deleted_at: false } }, include: { product: { select: { name: true, alert_quantity: true } } } },
        storeMaterials: { where: { deleted_at: false, material: { deleted_at: false } }, include: { material: { select: { name: true, unit: true, alert_quantity: true } } } },
      },
    });
    if (!store) return res.status(404).json({ error: 'Store not found' });

    const materials = store.storeMaterials || [];
    ...
    if (error.status === 403) return res.json({ materials: { count: 0, totalStock: 0, totalScrap: 0 }, products: { count: 0, totalStock: 0, totalScrap: 0 }, lowStock: { materials: [], products: [] } });
    res.status(500).json({ error: 'Failed to fetch store inventory summary' });
  }
});
...
router.put('/:id',  async (req, res) => {
  ...
  try {
    ...
    // Soft-delete existing store materials and products, then revive/create submitted rows
    ...
    await prisma.$transaction([
      ...(storeMaterials || []).map((material) =>
        prisma.storeMaterial.upsert({
          where: { store_id_material_id: { store_id: parseInt(id), material_id: material.material_id } },
          update: { stock: material.stock, deleted_at: false },
          create: { store_id: parseInt(id), material_id: material.material_id, stock: material.stock, deleted_at: false },
        })
      ),
      ...(storeProducts || []).map((product) =>
        prisma.storeProduct.upsert({
          where: { store_id_product_id: { store_id: parseInt(id), product_id: product.product_id } },
          update: { stock: product.stock, deleted_at: false },
          create: { store_id: parseInt(id), product_id: product.product_id, stock: product.stock, deleted_at: false },
        })
      ),
    ]);
